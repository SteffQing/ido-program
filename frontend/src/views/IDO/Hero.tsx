...
type Props = Omit<IDOType, "token_info" | "pool_info">;

function Hero(props: Props) {
  const progress = Math.round((props.raised / props.target) * 100);
  return (
    <aside className="flex flex-col gap-4 min-[950px]:flex-row">
      <div className="w-full flex-1 max-[950px]:max-w-[600px]">
        ...
        <h4 className="font-inter font-normal text-base md:text-xl border border-primary-foreground p-5 backdrop-blur-[1px]">
          {props.description}
        </h4>
        <div className="border border-primary-foreground border-t-0 w-[98%] p-1" />
      </div>
      <div className="w-full flex-1 p-5 border border-primary-foreground backdrop-blur-[3px] max-[950px]:max-w-[600px]">
